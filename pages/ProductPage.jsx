...
export default function ProductPage() {
  ...
  function handleAddToCart() {
    if (window.__addToCart) {
      for (let i = 0; i < qty; i++) window.__addToCart(product.name, product.price, mainImg || product.image);
      setAdded(true);
  ...
  return (
    <Layout>
          ...
          {variants.length > 0 && (
            <div className="variants-wrapper">
              ...
              <div className="variant-list">
                {variants.map((v, i) => (
                  <button key={i}
                    className={`variant-btn${activeV === i ? ' active' : ''}`}
                    onClick={() => { setMainImg(v.img); setActiveV(i); }}>
                    <img src={v.img} alt={`color-${i}`}
                      onError={e => e.target.style.opacity = 0.2} />
      ...
      <style>{`
        ...
        .btn-large{padding:15px 30px;font-size:18px;background:var(--primary);color:white;border:none;border-radius:5px;cursor:pointer;transition:0.3s;display:inline-flex;align-items:center;gap:10px;}
